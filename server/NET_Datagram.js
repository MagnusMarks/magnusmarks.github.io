...
Datagram.GetMessage = function(sock) {
	...
	for (; sock.messages.length > 0; ) {
		...
		if ((flags & 16) !== 0) {
			...
			var dest = new Uint8Array(NET.message.data);
		...
		if ((flags & 1) !== 0) {
			sock.driverdata.send(Buffer.from([0, 2, 0, 8, sequence >>> 24, (sequence & 0xff0000) >>> 16, (sequence & 0xff00) >>> 8, (sequence & 0xff) >>> 0]), 0, 8, sock.addr[1], sock.addr[0]);
